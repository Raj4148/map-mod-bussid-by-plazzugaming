...
export default function MapReady() {
  ...
  return (
    <PageShell>
      <div className="sticky top-0 z-40 bg-background/95 border-b border-border px-4 py-3 flex items-center justify-center">
        <h1 className="text-foreground font-black text-xs uppercase tracking-tighter">Your file is ready: {map.name}</h1>
      </div>

      <div className="px-4 pt-6 pb-20 flex flex-col items-center text-center">
        <div className="w-full max-w-md mx-auto space-y-8">
          <div className="bg-[#0f172a] rounded-[3rem] p-10 border border-white/5 shadow-2xl overflow-hidden relative group">
            <h3 className="text-white font-black text-xl mb-8 tracking-tighter uppercase">START YOUR DOWNLOAD</h3>
            <div className="relative aspect-video rounded-[2rem] overflow-hidden border-4 border-blue-400/20 mb-8 shadow-inner bg-slate-900">
              <SafeImage src={map.thumbnail} alt={map.name} className="w-full h-full object-cover" />
            </div>
            <div className="space-y-2 text-white">
              <p className="font-black text-sm uppercase tracking-tight line-clamp-1">{map.name} MOD</p>
              <div className="flex items-center justify-center gap-2 opacity-60">
                 <span className="text-[10px] font-bold uppercase tracking-widest">Type: .zip</span>
                 <div className="w-1 h-1 bg-white rounded-full" />
                 <span className="text-[10px] font-bold uppercase tracking-widest">Server: High Speed 5G</span>
              </div>
            </div>
          </div>

          <div className="space-y-4 px-2">
            <button onClick={handleDownload} className="w-full py-6 rounded-[1.5rem] bg-[#00ff88] text-[#0f172a] font-black text-xl shadow-[0_0_30px_rgba(0,255,136,0.4)] hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3">
              DOWNLOAD NOW <DownloadCloud className="w-6 h-6" />
            </button>

            <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest opacity-40">Fast download server selected by default</p>
          </div>
